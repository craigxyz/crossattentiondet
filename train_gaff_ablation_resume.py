...
"""
Enhanced training script for GAFF fusion ablation experiments with resume capability.

This script extends the original train_gaff_ablation.py with checkpoint resuming.
...
Usage:
    python train_gaff_ablation_resume.py \
        ...
        --output-dir results/gaff_ablations/exp_010 \
...
class Logger:
    ...
    def __init__(self, output_dir, resume_mode=False):
        ...
        # Open training log (append if resuming)
        mode = 'a' if resume_mode else 'w'
        self.log_file = open(self.training_log, mode, buffering=1)

        # Initialize CSV files if not resuming
        if not resume_mode:
...
class GAFFAblationTrainer:
    """Trainer for GAFF ablation experiments with resume capability."""

    def __init__(self, config, gaff_stages, gaff_se_reduction, gaff_inter_shared, gaff_merge_bottleneck, logger, resume_from_checkpoint=False):
        self.config = config
        ...
        # Log model info (skip if resuming)
        if not resume_from_checkpoint:
            self.model_info = self.logger.log_model_info(self.model)
        else:
            # Load model info from existing file
            model_info_path = os.path.join(logger.output_dir, 'model_info.json')
    ...
    def _resume_from_checkpoint(self):
        ...
        metrics_path = os.path.join(self.logger.output_dir, 'metrics_per_epoch.csv')

        if os.path.exists(checkpoint_path) and os.path.exists(metrics_path):
            self.logger.log_header("RESUMING FROM CHECKPOINT")
            self.logger.log(f"Loading checkpoint from: {checkpoint_path}")

            # Load model weights
            self.model.load_state_dict(torch.load(checkpoint_path, map_location=self.device))

            # Determine start epoch from metrics CSV
            with open(metrics_path, 'r') as f:
                ...
                if rows:
                    ...
                    self.best_loss = float(rows[-1]['train_loss'])

                    self.logger.log(f"Resuming from epoch {last_epoch}")
                    self.logger.log(f"Will continue from epoch {self.start_epoch}")
                    self.logger.log(f"Best loss so far: {self.best_loss:.4f}")
        else:
            self.logger.log("Warning: Resume requested but no checkpoint found, starting fresh")
    ...
    def train(self):
        ...
        if not self.resume_from_checkpoint:
            ...
            self.logger.log_header("TRAINING RESUME")
...
def main():
    parser = argparse.ArgumentParser(description='GAFF Fusion Ablation Training with Resume Support')
    ...
    # Resume option
    parser.add_argument('--resume-from-checkpoint', type=str, default='false',
                        choices=['true', 'false'],
    ...
    # Initialize logger
    logger = Logger(args.output_dir, resume_mode=resume_from_checkpoint)
    ...
    # Log configuration (skip if resuming)
    if not resume_from_checkpoint:
        config_dict = {
